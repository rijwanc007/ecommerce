        ...
        if($request->ajax()){
            if(permission('menu-bulk-delete')){
                ...
                if(!empty($delete_list)){
                        ...
                        $output = $result ?  ['status'=>'success','message'=> 'Selected Data Has Been Deleted Successfully. '. (!empty($undelete_list) ? 'Except these menus('.implode(',',$undelete_list).')'.' because they are associated with others data.' : '')] : ['status'=>'error','message'=>'Failed To Delete Data.'];
                }else{
                    $output = ['status'=>'error','message'=> !empty($undelete_list) ? 'These menus('.implode(',',$undelete_list).')'.' can\'t delete because they are associated with others data.' : ''];
                }
            }else{
                $output = $this->unauthorized();
            }
