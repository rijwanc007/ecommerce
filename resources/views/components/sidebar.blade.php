<div class="aside aside-left  aside-fixed  d-flex flex-column flex-row-auto" id="kt_aside">
    <div class="brand flex-column-auto " id="kt_brand">
        <div class="brand-logo">
            ...
            <a href="{{ route('dashboard') }}"><img src="{{ asset('storage/'.LOGO_PATH.config('settings.logo'))}}" style="max-width: 150px;" alt="Logo" /></a>
            @else
                    ...
                    @else
                        ...
                        " aria-haspopup="true" data-menu-toggle="hover">
                            <a href="javascript:void();" class="menu-link menu-toggle">
                                <span class="svg-icon menu-icon"><i class="{{ $menu->icon_class }}"></i></span>
                                <span class="menu-text">{{ $menu->module_name }}</span><i class="menu-arrow"></i>
                            </a>
                                    ...
                                    @foreach ($menu->children as $submenu)
                                        @if($submenu->children->isEmpty())
                                            <li class="menu-item {{ (request()->is($submenu->url)) ? 'menu-item-active' : '' }}" aria-haspopup="true">
                                                <a href="{{ $submenu->url ? url($submenu->url) : '' }}" class="menu-link "><i class="menu-bullet menu-bullet-dot"><span></span></i><span class="menu-text">{{ $submenu->module_name }}</span></a>
                                            </li>
                                        @else
                                            <li class="menu-item  menu-item-submenu
                                                @foreach ($submenu->children as $sub_submenu)
                                                ...
                                                @endforeach
                                                " aria-haspopup="true" data-menu-toggle="hover">
                                                <a href="javascript:void();" class="menu-link menu-toggle"><i class="menu-bullet menu-bullet-dot"><span></span></i><span class="menu-text">{{ $submenu->module_name }}</span><i class="menu-arrow"></i></a>
                                                <div class="menu-submenu ">
                                                    <span class="menu-arrow"></span>
                                                    <ul class="menu-subnav">
                                                        @foreach ($submenu->children as $sub_submenu)
                                                        <li class="menu-item {{ (request()->is($sub_submenu->url)) ? 'menu-item-active' : '' }}" aria-haspopup="true">
                                                            <a href="{{ $sub_submenu->url ? url($sub_submenu->url) : '' }}" class="menu-link "><i class="menu-bullet menu-bullet-dot"><span></span></i><span class="menu-text">{{ $sub_submenu->module_name }}</span></a>
                                                        </li>
                                                        @endforeach
                                                    </ul>
                                                </div>
